[SystemInformation]@{
    ...
    HardwareInformation = [HardwareInformation]@{
        ...
        FreeDiskSpace = $null # "{0:N1} GB" -f ((Get-PSDrive | Where-Object { $_.Name -eq '/' }).Free / 1GB)
        FreePhysicalMemory = $null#"{0:N1} GB" -f (( -split (Get-Content /proc/meminfo | Where-Object { $_ -match 'MemFree:' }))[1] / 1MB)
    }
}
