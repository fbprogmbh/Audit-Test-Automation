...
[AuditTest] @{
    Id = "18.8.7.1.5"
    Task = "(BL) Ensure 'Prevent installation of devices using drivers that match these device setup classes: Prevent installation of devices using drivers for these device setup' is set to 'IEEE 1394 device setup classes' ({d48179be-ec20-11d1-b6b8-00c04fa372a7})"
    Test = {
        try {
            $regValue = Get-ItemProperty -ErrorAction Stop `
...
[AuditTest] @{
    Id = "18.9.47.5.1.2 L"
    Task = "(L1) Ensure 'Configure Attack Surface Reduction rules: Block persistence through WMI event subscription' is configured"
    Test = {
            try {
            $regValue = 0;
            $regValueTwo = 0;
...
[AuditTest] @{
    Id = "19.7.43.1"
    Task = "(L1) Ensure 'Always install with elevated privileges' is set to 'Disabled' (CurrentUser)"
    Test = {
        try {
            $regValue = Get-ItemProperty -ErrorAction Stop `
