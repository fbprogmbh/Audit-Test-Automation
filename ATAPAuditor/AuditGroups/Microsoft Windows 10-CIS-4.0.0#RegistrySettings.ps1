...
[AuditTest] @{
    Id   = "18.6.4.2"
    Task = "(L1) Ensure 'Configure NetBIOS settings' is set to 'Enabled: Disable NetBIOS name resolution on public networks' or 'Enabled: Disable NetBIOS name resolution'"
    Test = {
        try {
            $regValue = Get-ItemProperty -ErrorAction Stop `
...
[AuditTest] @{
    Id   = "18.10.80.2"
    Task = "(L1) Ensure 'Allow Windows Ink Workspace' is set to 'Enabled: On, but disallow access above lock' OR 0 - 'Enabled: Disabled'"
    Test = {
        try {
            $regValue = Get-ItemProperty -ErrorAction Stop `
                -Path "Registry::HKEY_LOCAL_MACHINE\SOFTWARE\Policies\Microsoft\WindowsInkWorkspace" `
...
[AuditTest] @{
    ...
    Task = "(L2) Ensure 'Configure Brute-Force Protection aggressiveness' is set to 'Enabled: Medium' or higher"
    Test = {
        try {
            $regValue = Get-ItemProperty -ErrorAction Stop `
...
[AuditTest] @{
    ...
    Task = "(L2) Ensure 'Configure how aggressively Remote Encryption Protection blocks threats' is set to 'Enabled: Medium' or higher"
    Test = {
        try {
            $regValue = Get-ItemProperty -ErrorAction Stop `
