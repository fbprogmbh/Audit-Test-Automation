...
[AuditTest] @{
    Id = "2.2.14 A"
    Task = "(L1) Ensure 'Create symbolic links' is set to 'Administrators' [Hyper-V-Feature NOT installed]"
    Test = {
        $securityPolicy = Get-AuditResource "WindowsSecurityPolicy"
...
[AuditTest] @{
    Id = "2.2.16"
    Task = "(L1) Ensure 'Deny access to this computer from the network' to include 'Guests'"
    Test = {
        $securityPolicy = Get-AuditResource "WindowsSecurityPolicy"
...
[AuditTest] @{
    Id = "2.2.20"
    Task = "(L1) Ensure 'Deny log on through Remote Desktop Services' to include 'Guests'"
    Test = {
        $securityPolicy = Get-AuditResource "WindowsSecurityPolicy"
...
[AuditTest] @{
    Id = "2.2.25"
    Task = "(L1) Ensure 'Increase scheduling priority' is set to 'Administrators, Window Manager\Window Manager Group'"
    Test = {
        $securityPolicy = Get-AuditResource "WindowsSecurityPolicy"
...
[AuditTest] @{
    Id = "2.2.29 A"
    Task = "(L2) Ensure 'Log on as a service' is configured [Hyper-V-Feature NOT installed]"
    Test = {
        $securityPolicy = Get-AuditResource "WindowsSecurityPolicy"
...
[AuditTest] @{
    Id = "2.2.29 B"
    Task = "(L2) Ensure 'Log on as a service' is configured [Hyper-V-Feature installed]"
    Test = {
        $securityPolicy = Get-AuditResource "WindowsSecurityPolicy"
$currentUserRights = $securityPolicy["Privilege Rights"]["SeServiceLogonRight"]
$identityAccounts = @(
