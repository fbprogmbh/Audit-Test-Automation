...
[AuditTest] @{
    Id   = "18.9.30.1.1"
    Task = "(L1) Ensure 'Block NetBIOS-based discovery for domain controller location' is set to 'Enabled'"
    Test = {
        try {
            $regValue = Get-ItemProperty -ErrorAction Stop `
                -Path "Registry::HKEY_LOCAL_MACHINE\SOFTWARE\Policies\Microsoft\Netlogon\Parameters" `
                -Name "BlockNetbiosDiscovery" `
            | Select-Object -ExpandProperty "BlockNetbiosDiscovery"
...
[AuditTest] @{
    Id   = "18.9.39.1"
    Task = "(L1) Ensure 'Configure SAM change password RPC methods policy' is set to 'Enabled: Block all change password RPC methods'"
    Test = {
        try {
            $regValue = Get-ItemProperty -ErrorAction Stop `
            ...
            | Select-Object -ExpandProperty "SamrChangeUserPasswordApiPolicy"
