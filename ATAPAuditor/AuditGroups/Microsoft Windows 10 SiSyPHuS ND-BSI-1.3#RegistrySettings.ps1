...
[AuditTest] @{
    Id = "12"
    Task = "(ND, NE) Ensure 'MSS: (EnableICMPRedirect) Allow ICMP redirects tooverride OSPF generated routes' is set to 'Disabled'."
    Test = {
        try {
            $regValue = Get-ItemProperty -ErrorAction Stop `
                -Path "Registry::HKEY_LOCAL_MACHINE\System\CurrentControlSet\Services\Tcpip\Parameters" `
...
[AuditTest] @{
    Id = "172_1"
    Task = "(ND, NE) Ensure 'Configure Attack Surface Reduction rules: Set the state for each ASR rule' is 'configured'."
    Test = {
        try {
            $defStatus = (Get-MpComputerStatus -ErrorAction Ignore | Select-Object AMRunningMode)
            if ($defStatus.AMRunningMode -ne "Normal") {
...
[AuditTest] @{
    Id = "180"
    Task = "(ND, NE) Ensure 'Always install with elevated privileges' is set to 'Disabled' on current_user on local_machine."
    Test = {
        try {
            $regValue = Get-ItemProperty -ErrorAction Stop `
                -Path "Registry::HKEY_LOCAL_MACHINE\Software\Policies\Microsoft\Windows\Installer" `
...
[AuditTest] @{
    ...
    Test = {
        ...
        $expectedValue = 1;
        $profileType = "Private"
        $result = $path1, $path2 | Test-FirewallPaths -Key $key -ExpectedValue $expectedValue -ProfileType $profileType -ProfileType $profileType
        return @{
            Message = $($result.Message)
            Status = $($result.Status)
        }
