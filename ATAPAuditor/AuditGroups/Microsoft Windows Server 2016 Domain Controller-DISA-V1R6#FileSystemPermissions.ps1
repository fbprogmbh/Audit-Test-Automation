...
[AuditTest] @{
    ...
    Test = {
        $acls = (Get-Acl "${Env:ProgramFiles}\").Access
        
        Write-Verbose "File system permissions for TARGET: ${Env:ProgramFiles}\)"
...
[AuditTest] @{
    ...
    Task = "Permissions for program file directories must conform to minimum requirements."
    Test = {
        $acls = (Get-Acl "${Env:ProgramFiles(x86)}\").Access
        
        Write-Verbose "File system permissions for TARGET: ${Env:ProgramFiles(x86)}\)"
