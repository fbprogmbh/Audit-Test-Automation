...
[AuditTest] @{
    ...
    Test = {
        try {
            ...
            if ($regValue -ne 1024000) {
                return @{
                    Message = "Registry value is '$regValue'. Expected: 1024000"
                    Status = "False"
                }
            }
        }
...
[AuditTest] @{
    ...
    Test = {
        try {
            $regValue = Get-ItemProperty -ErrorAction Stop `
                -Path "Registry::HKEY_LOCAL_MACHINE\SOFTWARE\Policies\Microsoft\MicrosoftEdge\Privacy\" `
