...
[AuditTest] @{
    Id = "18.3.6"
    Task = "(L1) Set 'NetBIOS node type' to 'P-node' (Ensure NetBT Parameter 'NodeType' is set to '0x2 (2)') (MS Only)"
    Test = {
        try {
            $regValue = Get-ItemProperty -ErrorAction Stop `
...
[AuditTest] @{
    Id = "18.9.17.3"
    Task = "(L1) Ensure 'Disable OneSettings Downloads' is enabled."
    Test = {
        try {
            $regValue = Get-ItemProperty -ErrorAction Stop `
                -Path "Registry::HKEY_LOCAL_MACHINE\Software\Policies\Microsoft\Windows\DataCollection" `
