...
[AuditTest] @{
    ...
    Test = {
        try {
            ...
            if ($regValue.legalnoticetext -notmatch ".+" -or [string]::IsNullOrWhiteSpace($regValue.legalnoticetext) -or [string]::IsNullOrEmpty($regValue.legalnoticetext)) {
                return @{
...
[AuditTest] @{
    ...
    Test = {
        $path1 = "Registry::HKEY_LOCAL_MACHINE\Software\Policies\Microsoft\WindowsFirewall\PrivateProfile"
        $path2 = "Registry::HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\SharedAccess\Parameters\FirewallPolicy\PrivateProfile"       
        $key = "LogFilePath"
...
[AuditTest] @{
    Id = "19.7.43.1"
    Task = "(L1) Ensure 'Always install with elevated privileges' is set to 'Disabled' on current_user"
    Test = {
        try {
            $regValue = Get-ItemProperty -ErrorAction Stop `
