...
[AuditTest] @{
    Id = "18.6.4.1"
    Task = "(L1) Ensure 'Configure DNS over HTTPS (DoH) name resolution' is set to 2 - 'Enabled: Allow DoH' or higher"
    Test = {
        try {
            $regValue = Get-ItemProperty -ErrorAction Stop `
...
[AuditTest] @{
    Id = "18.6.4.2"
    Task = "(L1) Ensure 'Configure NetBIOS settings' is set to 2 - 'Enabled: Disable NetBIOS name resolution on public networks'"
    Test = {
        try {
            $regValue = Get-ItemProperty -ErrorAction Stop `
                -Path "Registry::HKEY_LOCAL_MACHINE\SOFTWARE\Policies\Microsoft\Windows NT\DNSClient" `
...
[AuditTest] @{
    Id = "18.9.5.2"
    Task = "(L1) Ensure 'Turn On Virtualization Based Security: Select Platform Security Level' is set to 'Secure Boot' or higher"
    Test = {
        try {
            $regValue = Get-ItemProperty -ErrorAction Stop `
                -Path "Registry::HKEY_LOCAL_MACHINE\SOFTWARE\Policies\Microsoft\Windows\DeviceGuard" `
...
[AuditTest] @{
    Id = "18.9.25.8"
    Task = "(L1) Ensure 'Post-authentication actions: Actions' is set to 'Enabled: Reset the password and logoff the managed account' or higher"
    Test = {
        try {
            $regValue = Get-ItemProperty -ErrorAction Stop `
                -Path "Registry::HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\LAPS" `
...
[AuditTest] @{
    Id = "18.10.16.1"
    Task = "(L1) Ensure 'Download Mode' is NOT set to 'Enabled: Internet (3)'"
    Test = {
        try {
            $regValue = Get-ItemProperty -ErrorAction Stop `
