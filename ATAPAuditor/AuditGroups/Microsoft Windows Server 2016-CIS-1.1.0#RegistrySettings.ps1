...
[AuditTest] @{
    ...
    Test = {
        try {
            ...
            if (($regValue -ne 15 -or $regValue -eq 0)) {
                return @{
    ...
    Test = {
        try {
            $regValue = Get-ItemProperty -ErrorAction Stop `
                ...
                -Name "MpEnablePus" `
                | Select-Object -ExpandProperty "MpEnablePus"
...
[AuditTest] @{
    Id = "18.5.4.2"
    Task = "(L1) Ensure 'Turn off multicast name resolution' is set to 'Enabled' (MS Only)"
    Test = {
        try {
            $regValue = Get-ItemProperty -ErrorAction Stop `
...
[AuditTest] @{
    Id = "18.8.28.1"
    Task = "(L1) Ensure 'Untrusted Font Blocking' is set to 'Enabled: Block untrusted fonts and log events'"
    Test = {
        try {
            $regValue = Get-ItemProperty -ErrorAction Stop `
