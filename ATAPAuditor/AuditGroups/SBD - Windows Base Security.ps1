...
[AuditTest] @{
	...
	Test        = {	
		...
		$status = switch ((Get-LocalUser).Count) {
			{ ($amountOfActiveUser -ge 0) -and ($amountOfActiveUser -le 2) } {
				# 0, 1, 2
				@{
					Message = "Compliant"
					Status  = "True"
			...
			{ ($amountOfActiveUser -gt 2) -and ($amountOfActiveUser -le 5) } {
				# 3, 4, 5
				@{
	...
	Test        = {	
		try { 
			...
			$status = switch ($amountOfUserAndGroups) {
				{ ($amountOfUserAndGroups -ge 0) -and ($amountOfUserAndGroups -le 2) } {
					# 0, 1, 2
					@{
				...
				{ ($amountOfUserAndGroups -gt 2) -and ($amountOfUserAndGroups -le 5) } {
					# 3, 4, 5
					@{
						Message = "Total amount of users: $amountOfUserAndGroups <br/> Amount of local users: $localCount <br/> Amount of domain users: $ADCount <br/>"
						Status  = "Warning"
