...
[AuditTest] @{
    Id = "18.3.5"
    Task = "(L1) Set registry value 'RestrictDriverInstallationToAdministrators' to 1."
    Test = {
        try {
            $regValue = Get-ItemProperty -ErrorAction Stop `
...
[AuditTest] @{
    Id = "18.5.4.1"
    Task = "(L1) Ensure 'Configure DNS over HTTPS (DoH) name resolution'  is set to 'Enabled: Allow DoH' or higher (Automated)"
    Test = {
        try {
            $regValue = Get-ItemProperty -ErrorAction Stop `
...
[AuditTest] @{
    Id = "18.9.47.5.1.2.4"
    Task = "(L1) Set the state for each Attack Surface Reduction (ASR) rule (Block Office applications from injecting code into other processes)."
    Test = {
        try {
            $regValue = Get-ItemProperty -ErrorAction Stop `
                -Path "Registry::HKEY_LOCAL_MACHINE\Software\Policies\Microsoft\Windows Defender\Windows Defender Exploit Guard\ASR\Rules" `
